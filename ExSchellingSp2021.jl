...
"""
#have to change rest of functions to only send "grid" to prop_dissatisfied instead of "grid + empty_loc"
...
function simulate_movement(pv, nrows, ncols, numsteps, rseed)
    ...
    #1
    pv = paramvalues()
    schgrid = initial_grid(nrows, ncols, pv)
    #2
    grid = schgrid[1]
    empty_locs = schgrid[2]
    #2a
    ...
    #3
    for i in 1:numsteps
        new_state!(grid, empty_locs, pv)
    end
    #4
    propdisend = prop_dissatisfied(grid, pv)
    dmend = calc_dm2(grid)
    return Dict(:propdisbeg => propdisbeg,
                :dmbeg => dmbeg,
...
function run_reps(numreps, pv, nrows, ncols, numsteps)
    ...
    for rseed = 1:numreps
        ...
        ## Your code here...
        #2
        s = simulate_movement(pv, nrows, ncols, numsteps, rseed)
        #3
        perc_chang_dm = 100 * ((s[:dmend] - s[:dmbeg])/s[:dmbeg])
        #4
        push!(df, [rseed, s[:propdisbeg], s[:dmbeg], s[:propdisend], s[:dmend], perc_chang_dm])
    end
...
function calcavgs(df)
    ## your code here...
    avg_propdisbeg = Sb.mean(df.propdisbeg)
    avg_dmbeg = Sb.mean(df.dmbeg)
    avg_propdisend = Sb.mean(df.propdisend)
    avg_dmend = Sb.mean(df.dmend)
    avg_perc_chng_dm = Sb.mean(df.perc_chng_dm)
    return Dict(:avg_propdisbeg => avg_propdisbeg,
                :avg_dmbeg => avg_dmbeg,
                :avg_propdisend => avg_propdisend,
                :avg_dmend => avg_dmend,
                :avg_perc_chng_dm => avg_perc_chng_dm )
end
pv = paramvalues()
# schgrid = initial_grid(25,25,pv)[1]
# prop_dissatisfied(schgrid, pv)
# print(simulate_movement(pv,25,25,15,100))
print(calcavgs(run_reps(3,pv,25,25,15)))
end ## end module
